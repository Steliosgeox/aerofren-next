...
const SilkPlane = forwardRef<Mesh, SilkPlaneProps>(function SilkPlane({ uniforms, paused = false }, ref) {
    ...
    useFrame((_state: RootState, delta: number) => {
        // Skip animation updates when paused (tab not visible)
        if (paused) return;
...
const Silk: React.FC<SilkProps> = ({ speed = 5, scale = 1, color = '#7B7481', noiseIntensity = 1.5, rotation = 0 }) => {
    ...
    return (
        ...
        >
            <SilkPlane ref={meshRef} uniforms={uniforms} paused={!isVisible} />
        </Canvas>
    );
};
