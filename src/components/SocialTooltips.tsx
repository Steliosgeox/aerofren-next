...
const StyledWrapper = styled.div<{ $isIntegrated?: boolean }>`
  ...
  .social-icons .icon-content .tooltip {
    ...
    background-color: var(--theme-tooltip-bg, #fff);
    color: var(--theme-tooltip-text, #000);
    padding: 6px 10px;
    border-radius: 8px;
    opacity: 0;
    visibility: hidden;
  ...
  .social-icons .icon-content .tooltip::before {
    content: "";
    position: absolute;
    bottom: 100%;
    left: 50%;
    ...
    border-bottom-color: var(--theme-tooltip-bg, #fff);
  }
  ...
  .social-icons .icon-content .link[data-social="twitter"]:hover {
    color: #1da1f2;
  }

  .social-icons .icon-content .link[data-social="mail"]:hover {
    color: var(--theme-accent, #0066cc);
  }
`;

export function SocialTooltips({ isIntegrated = false }: { isIntegrated?: boolean }) {
  return (
    <StyledWrapper $isIntegrated={isIntegrated}>
      <ul className="social-icons flex flex-row gap-2 justify-center items-center">
        <li className="icon-content">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30" className="w-5 h-5 fill-current">
