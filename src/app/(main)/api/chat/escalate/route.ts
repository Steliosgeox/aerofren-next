...
import { buildMessagePreview, getChatIdentityMetadata } from '@/lib/chat/session-metadata';
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (messagesSnapshot.empty) {
            ...
            const seedMessageRef = db.collection('chatMessages').doc();

            await seedMessageRef.set({
                sessionId,
                role: 'user',
                content:
                    'Ο χρήστης ζήτησε προώθηση σε εκπρόσωπο χωρίς προηγούμενο ιστορικό συνομιλίας.',
                timestamp: seedTimestamp,
                userId: decodedToken.uid,
                userEmail: decodedToken.email ?? 'Unknown',
                userName: decodedToken.name ?? decodedToken.email ?? 'Unknown User',
                isEscalated: true,
                escalatedAt: seedTimestamp,
            });
            ...
            await db.collection('chatSessions').doc(sessionId).set(
                {
                    ...
                    ...getChatIdentityMetadata({
                        ...
                        userName: decodedToken.name ?? decodedToken.email ?? undefined,
                        userPhotoURL: decodedToken.picture ?? undefined,
                    }),
            ...
            latestDocId = seedMessageRef.id;
            latestTimestamp = seedTimestamp.toDate();
        } else {
        ...
        let status: ChatEscalationStatus = 'pending';
        let alreadyEscalated = false;

        if (existingEscalation.exists) {
            const existingStatus = existingEscalation.data()?.status;
            if (
        ...
        await db.collection('chatSessions').doc(sessionId).set(
            {
                ...
                lastMessagePreview: buildMessagePreview(
                    messagesSnapshot.empty
                        ? 'Ο χρήστης ζήτησε προώθηση σε εκπρόσωπο χωρίς προηγούμενο ιστορικό συνομιλίας.'
                        : 'Ο χρήστης ζήτησε προώθηση σε εκπρόσωπο.'
                ),
                lastMessageRole: 'user',
                assignedAdminEmail: FieldValue.delete(),
                resolvedAt: FieldValue.delete(),
                resolvedBy: FieldValue.delete(),
                ...getChatIdentityMetadata({
                    userEmail: decodedToken.email ?? undefined,
